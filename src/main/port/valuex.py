...
class ValueX():
    ...
    def CalcAnnualReturnRate(self): # 006
        ...
        annual_return_rate = pow(self.daily_report.ix[self.daily_report.shape[0] - 1, "net_cumulative"] / self.daily_report.ix[0, "net_cumulative"], self.config.cfg_anal.trading_days_year / len(period_days)) - 1.0
        index_annual_return_rate = pow(self.daily_report.ix[self.daily_report.shape[0] - 1, "refer_index"] / self.daily_report.ix[0, "refer_index"], self.config.cfg_anal.trading_days_year / len(period_days)) - 1.0
        return annual_return_rate, index_annual_return_rate # 年化收益率、参照指数年化收益率

    def CalcReturnVolatility(self): # 007
        return_volatility = self.daily_report["daily_net_rise"].std() * math.sqrt(self.config.cfg_anal.trading_days_year)
        return return_volatility # 收益波动率

    def CalcSharpeRatio(self, annual_return_rate, return_volatility): # 008
        sharpe_ratio = (annual_return_rate - self.config.cfg_anal.benchmark_rate) / return_volatility
    ...
    def CalcAlphaValue(self, annual_return_rate, index_annual_return_rate, beta_value): # 010
        alpha_value = (annual_return_rate - self.config.cfg_anal.benchmark_rate) - beta_value * (index_annual_return_rate - self.config.cfg_anal.benchmark_rate)
    ...
    def CalcInfoRatio(self): # 011
        ...
        annual_diff_mean = return_rate_diff.mean() * self.config.cfg_anal.trading_days_year
        annual_diff_std = return_rate_diff.std() * math.sqrt(self.config.cfg_anal.trading_days_year)
