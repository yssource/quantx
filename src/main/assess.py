...
class Assess():
    ...
    def SaveDailyReport(self, data_file):
        ...
        if len(daily_report_list) > 0:
            ...
            if self.folder_assess == "": # 缓存路径为空
                self.log_text = "缓存数据 每日报表 时，本地数据缓存路径为空！"
                self.SendMessage("E", 4, self.log_cate, self.log_text, "A")
            else:
    ...
    def GetDailyReport(self, account, date_s, date_e):
        ...
        result = pd.DataFrame(columns = columns) # 空
        if self.folder_assess == "": # 缓存路径为空
            self.log_text = "直接缓存获取 每日报表 时，本地数据缓存路径为空！"
            self.SendMessage("E", 4, self.log_cate, self.log_text, "A")
        else:
            save_path = "%s/%s" % (self.folder_assess, self.sheet_daily_report)
            if not os.path.exists(save_path): # 缓存文件不存在
                self.log_text = "直接缓存获取 每日报表 时，本地数据缓存文件不存在！"
                self.SendMessage("E", 4, self.log_cate, self.log_text, "A")
            else: # 读取缓存文件
                result = pd.read_pickle(save_path)
                self.log_text = "本地缓存 获取 %d 条 每日报表 数据。" % result.shape[0]
                self.SendMessage("I", 1, self.log_cate, self.log_text, "A")
                ...
                self.log_text = "本地缓存 滤得 %d 条 每日报表 数据。" % result.shape[0]
                self.SendMessage("I", 1, self.log_cate, self.log_text, "A")
        if result.empty:
            self.log_text = "获取的 每日报表 为空！"
    ...
    def StrategyEvaluation(self, daily_report):
        ...
        if not daily_report.empty:
            self.valuex = valuex.ValueX(daily_report = daily_report)
            average_daily_net_rise = self.valuex.CalcAverageDailyNetRise() # 001
            max_period_return, min_period_return = self.valuex.CalcMaxMinPeriodReturn() # 002
            go_up_probability = self.valuex.CalcGoUpProbability() # 003
            max_days_keep_up, max_days_keep_down = self.valuex.CalcMaxDaysKeepUpOrDown() # 004
            max_drawdown_value, max_drawdown_date, drawdown_start_date = self.valuex.CalcMaxDrawdown() # 005
            annual_return_rate, index_annual_return_rate = self.valuex.CalcAnnualReturnRate() # 006
            return_volatility = self.valuex.CalcReturnVolatility() # 007
            sharpe_ratio = self.valuex.CalcSharpeRatio(annual_return_rate, return_volatility) # 008
            beta_value = self.valuex.CalcBetaValue() # 009
            alpha_value = self.valuex.CalcAlphaValue(annual_return_rate, index_annual_return_rate, beta_value) # 010
            info_ratio = self.valuex.CalcInfoRatio() # 011
            ...
            self.valuex.MakeNetValueCompare(self.save_folder) # 012
