...
class Center(common.Singleton):
    ...
    def __init__(self):
        ...
        self.update_timestamp_stock_ltb = datetime.datetime.now() # 股票个股
        self.update_timestamp_index_ltb = datetime.datetime.now() # 股票指数
        self.update_timestamp_trans_ltb = datetime.datetime.now() # 股票逐笔
        self.update_timestamp_stock_ltp = datetime.datetime.now() # 股票个股
        self.update_timestamp_index_ltp = datetime.datetime.now() # 股票指数
        self.update_timestamp_trans_ltp = datetime.datetime.now() # 股票逐笔
        self.update_timestamp_stock_tdf = datetime.datetime.now() # 股票个股
        self.update_timestamp_index_tdf = datetime.datetime.now() # 股票指数
        self.update_timestamp_trans_tdf = datetime.datetime.now() # 股票逐笔
        self.update_timestamp_future_np = datetime.datetime.now() # 期货内盘
    ...
    def UpdateSnapshot(self, data_type, tmp_data):
        try:
            if data_type == define.stock_ltb_market_s_func:
                str_code = str(tmp_data[0]) # 600000, 000001
                ...
                pub.sendMessage("center.quote.stock_ltb", src_data)
                self.quote_sub_locker_stock_ltb.acquire()
                ...
                str_code = str(tmp_data[0]) # 399001, 000001
                ...
                pub.sendMessage("center.quote.trans_ltb", src_data)
                self.quote_sub_locker_trans_ltb.acquire()
                ...
                pub.sendMessage("center.quote.stock_ltp", src_data)
                self.quote_sub_locker_stock_ltp.acquire()
                ...
                pub.sendMessage("center.quote.index_ltp", src_data)
                self.quote_sub_locker_index_ltp.acquire()
                ...
                pub.sendMessage("center.quote.trans_ltp", src_data)
                self.quote_sub_locker_trans_ltp.acquire()
                ...
                pub.sendMessage("center.quote.index_tdf", src_data)
                self.quote_sub_locker_index_tdf.acquire()
                ...
                pub.sendMessage("center.quote.future_np", src_data)
                self.quote_sub_locker_future_np.acquire()
                for strategy, quote_func in self.quote_sub_dict_future_np.items():
                    quote_func(self.DataMsg(src_data))
                self.quote_sub_locker_future_np.release()
        except Exception as e:
            print("UpdateSnapshot Error:", Exception, e)
            traceback.print_exc()
            info = sys.exc_info()
